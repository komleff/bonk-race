...
Инструмент для настройки физики движения. Отдельная страница `/lab`, ~50 параметров в реальном времени.

**Возможности:**
- 60Hz Newtonian physics (F=ma, semi-implicit Euler, Flight Assist)
- Интерполяция рендера между тиками физики (плавное движение на 120+ Гц)
...
- Следы движения: 3 паттерна (моноцвет, по дрифту, радуга), colour picker
- Шипы: knockback + destroy опции
- Орбы — баллистические объекты, столкновения со всем
...
- **v0.6.0** — Интерполяция рендера, единый RAF-цикл, плавный след
- **v0.5.0** — Рефакторинг BonkLab, тюнинг зон, балансировка арены
- **v0.4.0** — Следы движения, треугольник направления, зонные слайдеры
- **v0.3.0** — Анизотропное трение (Lateral Grip), 4 поверхности
