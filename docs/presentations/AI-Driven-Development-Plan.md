...
## Часть 8: Где мы на карте и куда дальше

> На основе статьи [«8 уровней агентной инженерии»](https://habr.com/p/1010430/)

### 8.1 Восемь уровней агентной инженерии
...
### 8.2 Наш профиль зрелости: уверенный 4, частично 5–6

Маппинг практик проекта на уровни:

| Уровень | Статус | Наши практики |
|---------|--------|---------------|
| 3 — Управление контекстом (Context Engineering) | ✅ Освоен | CLAUDE.md, AGENT_ROLES.md, Memory Bank |
| 4 — Накопление опыта (Compounding Engineering) | ✅ Освоен | Memory Bank фиксирует уроки, rules обновляются между сессиями |
| 5 — Инструменты и навыки (MCP и Skills) | 🟡 Частично | Интеграция локальных инструментов через MCP (Beads); кастомные скиллы на паузе |
| 6 — Обвязка и обратная связь (Harness Engineering) | 🟡 Частично | Zero Trust (CI: build → test → review), детерминизм-тесты, мультивендорное ревью |

**Чего не хватает до полного 6-го уровня:**

- Цикл обратной связи не замкнут — человек сливает PR и копирует промпты между агентами
- Нет автоматического противодавления (backpressure) — агент пока не обнаруживает и не исправляет ошибки самостоятельно

### 8.3 Следующий шаг: уровень 7 — фоновые агенты (Background Agents)

| Что нужно | Текущий статус | Связь с Roadmap |
|-----------|---------------|-----------------|
| Оркестратор задач | ❌ Ручная координация | → Beads Orchestrator (Часть 7.4) |
| Автоматическое слияние кода | ❌ Человек сливает | → Пайплайн Task → Agent → Review → Merge |
| Подбор модели под задачу | 🟡 Частично (Opus/Sonnet/Gemini) | → Dispatcher выбирает модель |
| Фоновая работа без человека | ❌ Все сессии интерактивные | → Агенты работают автономно по расписанию |

Уровень 7 совпадает с нашим Roadmap из Части 7.4.

### 8.4 Итог

Это не финальная точка — это позиция на растущей шкале. Уровни 3–5 — фундамент. Без него переход к автономным агентам (7–8) невозможен.
...
| **Background Agents** | Фоновые агенты — асинхронно выполняют задачи без участия человека |
| **Compounding Engineering** | Практика фиксации уроков в rules-файлы для накопительного эффекта |
| **Harness Engineering** | Построение обвязки с автоматической обратной связью (тесты, линтеры, CI) |
| **MCP** | Model Context Protocol — протокол подключения внешних инструментов к языковой модели |
| **Skills** | Кастомные навыки агента — предопределённые действия, вызываемые по команде |
