...
## Слайд 17: Проблемы — заголовок

**Контент:**
- Крупный заголовок: «Что не удалось победить»
- Подзаголовок: «Честный разбор за 3 месяца»

**Визуал:**
- Персонаж-аватар с выражением "задумчивости"
- Фон: тёмный с красным/оранжевым акцентом
- Иконка warning

---

## Слайд 18: Проблема — итерационный цикл
...
Код → Ревью → Фикс → ❌ СТОП
                       ↑
              нужно ещё 3-7 итераций
```

**Пример:** PR #5 — GPT нашёл coin farm (P0). Developer "исправил". Итерация 3 — тот же баг на месте.

**Причина:** субагент запускается в чистом контексте, не видит историю

**Визуал:**
- Схема цикла, где после первого фикса — красный крестик
- Рядом — идеальная схема с 5 итерациями
- Цитата из реального ревью (скриншот комментария)

---

## Слайд 19: Проблема — английский в комментариях

**Контент:**

Правило прописано в 3 местах:
- CLAUDE.md ✅
- AGENT_ROLES.md ✅
- copilot-instructions.md ✅

Результат: **~70% compliance**

PR #21 — "enforce Russian code comments" — отдельный PR только на язык

**Причина:** модели обучены на англоязычном коде

**Визуал:**
- Скриншот кода с английским комментарием, зачёркнутым красным
- Рядом — правильный вариант на русском
- Процент compliance: большой "70%" с предупреждением

---

## Слайд 20: Проблема — God Objects
...
## Слайд 21: Проблема — безопасность

**Контент:**

3 реальных уязвимости, найденных GPT-5.3:

| Уязвимость | Приоритет | Исправлено за 3 итерации? |
|-----------|-----------|--------------------------|
| Coin farm — клиент контролирует награды | P0 | ❌ Нет |
| Нет идемпотентности — дублирование наград | P1 | ❌ Нет |
| Guest auth → 500 error | P1 | Частично |

**Вывод:** кросс-ревью разными моделями — необходимость для безопасности

**Визуал:**
- Таблица с красными крестиками
- Иконки безопасности
- Подпись: «Один ревьювер = недостаточно»

---

## Слайд 22: Проблема — инфраструктура

**Контент:**

Docker:
- v0.3.1 — ошибка конфига (withdrawn)
- v0.3.2 — ESM/CommonJS (withdrawn)
- v0.3.3 — 9 фиксов

Redis в продакшене:
- `stop-writes-on-bgsave-error` не настроен
- **22 часа даунтайма**

**Вывод:** DevOps ≠ задача для агентов

**Визуал:**
- Timeline с тремя провалами Docker
- Красная метка "22h downtime" с иконкой сервера
- Подпись: «Не доверяйте агентам продакшен-инфраструктуру»

---

## Слайд 23: Проблема — Gemini как ревьювер

**Контент:**

Промпт:
> «Ты ТОЛЬКО ревьюишь. Ты НЕ правишь код.»

Gemini: *правит код*

| Модель | Следует роли ревьювера? | Находит реальные баги? |
|--------|------------------------|----------------------|
| GPT-5.4 | ✅ Да | ✅ Да |
| GPT-5.3 | ✅ Да | ✅ Да |
| Gemini | ❌ Нет | ❌ Ложно-положительные |

**Визуал:**
- Диалоговый пузырь с промптом
- Ниже — "ожидание" vs "реальность" (два блока)
- Таблица с зелёными/красными маркерами

---

## Слайд 24: Сводка проблем

**Контент:**

| Проблема | Статус | Workaround |
|---------|--------|------------|
| Цикл ревью обрывается | ❌ Не решено | Ручной запуск итераций |
| Английский в комментариях | ⚠️ 70% | Отдельный PR на исправление |
| God Objects | ⚠️ Планируется рефакторинг | Осторожные изменения |
| Безопасность | ⚠️ Кросс-ревью помогает | 3+ ревьювера обязательно |
| Инфраструктура | ❌ | Человеческий контроль |
| Gemini неуправляем | ❌ | Не доверять вердиктам |
| Агенты не учатся | ❌ | Передавать контекст вручную |

**Визуал:**
- Компактная таблица с цветовыми статусами
- Общий тон: честный, не пессимистичный — "мы знаем проблемы и работаем над ними"

---

## Слайд 25: Три столпа пайплайна
...
## Слайд 27: 8 уровней агентной инженерии
...
## Слайд 28: Эмоциональная кривая ИИ-разработки
...
## Слайд 29: Стартер-кит — что получаете
...
## Слайд 30: Три уровня погружения
...
## Слайд 34 (бонус): Заготовленные ответы на Q&A
