...
1. Скопирует все файлы (включая скрытые `.agents/`, `.claude/`, `.memory_bank/` и др.)
2. Переименует шаблоны (уберёт суффикс `.template`)
3. Установит глобальные настройки Claude в `~/.claude/settings.json`
4. Уже существующие файлы не будут перезаписаны

После установки:

- Замените плейсхолдеры (см. таблицу ниже)
- Заполните Memory Bank
- Запустите `claude` — агент должен поздороваться на русском
